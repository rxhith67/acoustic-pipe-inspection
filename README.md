...
2. **Signal processing** (`src/features.py`) — STFT spectrograms (2D time-frequency images), z-score normalised waveforms, sliding-window envelope features, and classical peak detection used as a DSP baseline.

3. **Blockage Detector** (`src/models.py`) — 2D CNN trained on spectrograms. Binary classification: clear pipe vs. blocked. Uses Focal Loss (α=0.25, γ=2.0) with inverse-frequency class weighting to handle imbalanced datasets.

4. **Blockage Localiser** (`src/models.py`) — 1D CNN trained on raw waveforms. Regresses the normalised position of the primary blockage along the pipe. Trained with a physics consistency loss (λ=0.1) that penalises predictions inconsistent with DSP-derived echo timing: $d = vt/2$.
...
Open notebooks 01–05 in order for full visualisations, analysis, and real-data validation.
...
- **Simulation-to-reality gap**: the synthetic simulator uses simplified physics (1D wave propagation, pink noise). Real pipes have bends, joints, and complex reverb. Fine-tuning on actual NBN Co pipe recordings would substantially improve real-world performance.
